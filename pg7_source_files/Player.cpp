...
bool Player::takeCamels(Game g)
{
    ...
    for (int i = 0; i < 5; i++) {
        if (g.market.getCard(i).getType() == "Camels") {
            camelsInMarket++;
            myHerd.addCard(g.market.at(i));	///puts camel in herd
...
bool Player::sellOne(Game g, int handInd)
{
    	...
    	if (isValidSaleOfOne(handInd)) {
			...
			else {std::cout<<"Error selling card"<<std::endl;}
			//free(c);
		}
		else{
...
bool Player::isValidSaleOfMult(class Game g, string tp) 
{
	int inHand=0;
...
{
	int numSold=0;
	firstInd=g.handIndicesForSelling.at(1);
	string tp=firstInd.getType();
	int hdInd=0;
	...
	//make sure more than one card of same type in hand
	//make sure more than one card in IndforSelling vector
	if(isValidSaleOfMult())
	{
		Token * bToken;
		///award bonus tokens
		int numSelling=g.handIndicesForSelling.size();
		...
		int howMany=g.handIndicesForSelling.size();
